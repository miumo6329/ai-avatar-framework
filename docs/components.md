...
稼働状態とタイムアウト・リトライの詳細は [timeout-resilience-design.md](timeout-resilience-design.md) を参照。

---

### 4. STTWorker

音声データをテキストに変換。処理パイプライン・アダプター設計・節区切り検出・内部状態の詳細は [stt-processing-design.md](stt-processing-design.md) を参照。

VAD（発話区間検出）はUnity Bridge側の責務であり、STTWorkerにはVAD済みの発話音声のみが届く。
...
コンテキスト構成要素（システムプロンプト・Bridge Capabilities・Perception Snapshot・RAG検索結果・会話履歴・現在の発話）とトークン配分優先度の詳細は [llm-conversation-design.md](llm-conversation-design.md) を参照。
...
データ構造・軽量モデル選定方針・処理フローの詳細は [llm-conversation-design.md](llm-conversation-design.md) を参照。

---

### 6. TTSWorker

テキストを音声に変換。詳細なバッファリング設計は [tts-buffering-design.md](tts-buffering-design.md) を参照。
...
状態遷移図・イベント優先度体系・割り込みフローの詳細は [llm-conversation-design.md](llm-conversation-design.md) を参照。
...
インターフェース定義・`PerceptionEntry` データ構造・センサーごとのTTL目安・能動発話制御の詳細は [llm-conversation-design.md](llm-conversation-design.md) を参照。
