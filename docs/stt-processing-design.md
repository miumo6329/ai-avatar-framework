...
## 設計方針：VADはAdapter側の責務

マイクデバイスはAdapterが管理するため、VADもAdapter側で実施する。
AdapterはVADで発話区間を検出し、**発話音声のみ**をCoreに送信する。無音区間はネットワーク転送しない。

```
Unity Adapter（VRヘッドセット等）        Python Core（ホストPC等）
──────────────────────────────           ──────────────────────────
[マイク]
  ↓ 生PCM音声（全区間）
  → VAD判定（Silero等）
...
Adapterの責務はマイク入力からVADを経て発話音声を送信すること。
...
ROS等の他Adapterも同様にAdapter側でVADを実装する。
各プラットフォームのネイティブ音声APIや既存VADライブラリを活用してよい。

### VADパラメータ

Adapter側の実装に委ねるが、以下を設定可能にすることを推奨:
...
Adapterから送信される `audio.input` の仕様。**発話区間のみ送信**する。
...
中間チャンク × N（省略可。Adapterが内部バッファに蓄積してもよい）
...
アバター発話中（SPEAKING状態）にユーザーが話し始めた場合、Adapterの割り込み検出が起動する。

### 割り込み検出フロー

```
Adapter: SPEAKING中に発話検出
...
ユーザー発話開始（Adapter側VAD検出）から `stt.final` がLLMWorkerに届くまでの遅延。
...
```
Adapter: 発話開始検出 → audio.input (is_speech_start=true) 送信
...
VAD設定はAdapter側に委ねるため、stt.yamlはSTTエンジン設定のみを記述する。
